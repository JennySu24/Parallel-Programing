...
void PriorityQueue::Generate(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        std::vector<std::string> local_lines(local_n);
        
        // OpenMP 多线程生成本地猜测
        #pragma omp parallel for
        for (int i = 0; i < local_n; ++i)
        {
            local_lines[i] = ordered_values[start + i] + '\n';
        }
        ...
        if (rank == 0)
        {
            ...
            for (int i = 0; i < size - 1; ++i)
            {
                ...
                while (std::getline(iss2, local_line))
                    local_guesses.push_back(local_line);
    ...
    else
    {
        ...
        std::vector<std::string> local_lines(local_m);
        
        // OpenMP 多线程生成本地猜测
        #pragma omp parallel for
        for (int i = 0; i < local_m; ++i)
        {
            local_lines[i] = prefix + last_values[start + i] + '\n';
        }
