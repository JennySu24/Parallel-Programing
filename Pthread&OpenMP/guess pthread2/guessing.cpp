...
void* PriorityQueue::Thread_worker(void* args) {
    ThreadArg* targs = (ThreadArg*)args;
    PriorityQueue* q = targs->pq;
    segment* a = targs->seg_ptr;
    PT* pt = targs->pt;
...
void PriorityQueue::Generate(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        for (int i = 0; i < num_threads; i++) {
            ...
            args->prefix = "";
            args->start = i * range;
            args->end = (i == num_threads-1) ? max_idx : (i+1)*range;

            pool->addTask(Thread_worker, args);
        }
